...
questionOne () {
  ...
  while [ "$ANS_CONTINUE" != "y" ] || [ "$ANS_CONTINUE" != "Y" ] || [ "$ANS_CONTINUE" != "n" ] || [ "$ANS_CONTINUE" != "N" ]; do
    ...
    if [ "$ANS_CONTINUE" == "y" ] || [ "$ANS_CONTINUE" == "Y" ]; then
      echo -e "\nSearching all subdirectories for Wordpress installs. Please wait...\n"
      wpConfigSearch
    fi
...
wpConfigSearch () {
  ...
  ## Find if the wp-config.php file exists within all subdirectories
  ...
  if [ -z "$EACH_WP_LOC" ]; then
    ...
    for each in "${EACH_WP_LOC[@]}"; do
      EACH_WP_NAME+=("$(grep -oP "^\s*define\s*\(\s*'DB_NAME'\s*,\s*'\K.+(?=')" $each/wp-config.php)")
      EACH_WP_PASS+=("$(grep -oP "^\s*define\s*\(\s*'DB_PASSWORD'\s*,\s*'\K.+(?=')" $each/wp-config.php)")
      EACH_WP_USER+=("$(grep -oP "^\s*define\s*\(\s*'DB_USER'\s*,\s*'\K.+(?=')" $each/wp-config.php)")
      EACH_WP_HOST+=("$(grep -oP "^\s*define\s*\(\s*'DB_HOST'\s*,\s*'\K.+(?=')" $each/wp-config.php)")
      ...
      if [ -w "$ANS_LOCATION" ]; then
        ...
        if [ "$ANS_INSTALLS" == 0 ]; then
          ...
          for each in "${ANS_INS_ARRAY[@]}"; do
            ((each=each-1))
            if [[ "$ANS_LOCATION" == "${EACH_WP_LOC[$each]}"* ]]; then
              BAD_DIR=true
            fi
          done
        fi
        if [ "$BAD_DIR" == true ]; then
          echo -e "The directory cannot reside in the website root directory. Please try another path or create a directory to compress to outside of the website root."
        else
...
## Ask user if they want all files or just core files
questionFour () {

  while [ -z "$WP_CORE" ]; do

    ## Ask where to store the backups
    echo -e "\nWould you like all files or just Wordpress core files?"
    read -p "(Enter 1 for CORE or ENTER for ALL): " ANS_CORE
  ...
  ## If user selected core then scan all core files to see if they exist
  if [ "$WP_CORE" == true ]; then

    ## Check for all core files in all Wordpress installs
    if [ "$ANS_INSTALLS" == 0 ]; then
      ii=0
      for each in "${EACH_WP_LOC[@]}"; do
        for i in "${WP_CORE_FILES[@]}"; do

          ## Check if it is a file or directory. If it exists add it to the array
          if [[ "$i" =~ \.+ ]]; then
            if [ -f "$each$i" ]; then
              WP_CORE_SELECT[$ii]+="$i "
            fi
    ...
    ## Check for all core files in selected Wordpress installs
    else
      ...
      for each in "${ANS_INS_ARRAY[@]}"; do
        ((each=each-1))
         for i in "${WP_CORE_FILES[@]}"; do
          ...
          ## Check if it is a file or directory. If it exists add it to the array
          if [[ "$i" =~ \.+ ]]; then
            if [ -f "${EACH_WP_LOC[$each]}$i" ]; then
              WP_CORE_SELECT[$each]+="$i "
            fi
          else
            if [ -d "${EACH_WP_LOC[$each]}$each$i" ]; then
              WP_CORE_SELECT[$each]+="$i "
            fi
          fi
        done
...
wpCompressor () {
  ...
  if [ "$ANS_INS_ARRAY" -eq 0 ]; then
    ...
    for i in "${EACH_WP_LOC[@]}"; do
      ...
      if [ "$WP_CORE" == true ]; then
        tar -czf ${APPROVED_LOCATION}_${EACH_WP_OURL[$ii]}.${NOW}.bak.tgz -C ${EACH_WP_LOC[$ii]} ${WP_CORE_SELECT[$ii]}
      else
  ...
  ## Compress selected
    ...
    for i in "${ANS_INS_ARRAY[@]}"; do
      ...
      if [ "$WP_CORE" == true ]; then
        tar -czf ${APPROVED_LOCATION}_${EACH_WP_OURL[$i]}.${NOW}.bak.tgz -C ${EACH_WP_LOC[$i]} . ${WP_CORE_SELECT[$i]}
      else
